...
***** 获取所有具有配置的MQ id
=GET /admin/mq-with-configurations=
...
curl \
  ...
  "http://localhost:8080/admin/mq-with-configurations"
#+END_SRC

******* fetch
#+BEGIN_SRC js
await fetch('http://localhost:8080/admin/mq-with-configurations', {
  headers: {
...
***** 获取特定队列的配置
=GET /admin/mq/<id>/configurations=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/mq/$id/configurations"
#+END_SRC

******* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/mq/${id}/configurations`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
***** 设置配置
=PUT /admin/mq/<mq-id>/configurations/unique=
=PUT /admin/mq/<mq-id>/configurations/drafting-timeout=
=PUT /admin/mq/<mq-id>/configurations/ordered-timeout=
=PUT /admin/mq/<mq-id>/configurations/active-timeout=
=PUT /admin/mq/<mq-id>/configurations/concurrency=
=PUT /admin/mq/<mq-id>/configurations/throttle=
...
curl \
  ...
  --header "Content-Type: application/json" \
  --data "$UNIQUE" \
  "http://localhost:8080/admin/mq/$id/configurations/unique"
#+END_SRC

******* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/mq/${id}/configurations/unique`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
...
***** 移除配置
=DELETE /admin/mq/<mq-id>/configurations/unique=
=DELETE /admin/mq/<mq-id>/configurations/drafting-timeout=
=DELETE /admin/mq/<mq-id>/configurations/ordered-timeout=
=DELETE /admin/mq/<mq-id>/configurations/active-timeout=
=DELETE /admin/mq/<mq-id>/configurations/concurrency=
=DELETE /admin/mq/<mq-id>/configurations/throttle=

...
import { AdditionalBehavior, DuplicateMessageId, MessageState, QueueNotFound } from '@src/contract.js'
...
describe('draftMessage', () => {
  ...
  describe('with message id', () => {
    test('queue does not exist', async () => {
      const client = await buildClient()
      const queueId = 'queue-id'

      const err = await getErrorAsync(() => client.draftMessage(
        queueId
      , null
      , ['slot']
      , 'message-id'
    ...
    describe('queue exists', () => {
      test('message does not exist', async () => {
        try {
          ...
          const priority = null
          const messageId = await client.draftMessage(
            queueId
          , priority
          , ['slot']
          , 'message-id'
    ...
    test('message exists', async () => {
      try {
        ...
        expect(err).toBeInstanceOf(DuplicateMessageId)
        expect(await client.getQueueStats(queueId)).toMatchObject({
          drafting: 1
        })
        expect(getRawMessage(queueId, messageId)).toMatchObject({
